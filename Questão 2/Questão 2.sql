...
#Consulta todas as consultas que estão finalizadas
CREATE TABLE consultas_finalizadas AS
SELECT pac.nome, con.observacoes, con.data_consulta, con.status_consulta
FROM paciente AS pac
INNER JOIN consulta AS con ON (pac.id = con.id_paciente)
WHERE con.status_consulta = 'Finalizada';

SELECT * FROM consultas_finalizadas;
